...
std_msgs__msg__Int32 msg_s1;

rcl_subscription_t subscriber_s2;
std_msgs__msg__Int32 msg_s2;


// 订阅者舵机S1回调函数
// Subscriber servo s1 callback function
...
// 订阅者舵机S2回调函数
// Subscriber servo s2 callback function
...
void micro_ros_task(void *arg)
{
    ...
    // create node
    rcl_node_t node;
    RCCHECK(rclc_node_init_default(&node, "servo_subscriber", ROS_NAMESPACE, &support));

    // 创建订阅者
    // Create subscriber_s1.
    ...
    RCCHECK(rclc_executor_add_subscription(&executor, &subscriber_s1, &msg_s1, &servo_s1_callback, ON_NEW_DATA));
    RCCHECK(rclc_executor_add_subscription(&executor, &subscriber_s2, &msg_s2, &servo_s2_callback, ON_NEW_DATA));
    ...
    // 释放资源
    // free resources
    RCCHECK(rcl_subscription_fini(&subscriber_s1, &node));
    RCCHECK(rcl_subscription_fini(&subscriber_s2, &node));
